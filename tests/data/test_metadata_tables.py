"""Snapshot the table set bound to ``agentbox.core.data.metadata``.
...
def test_metadata_is_shared_singleton() -> None:
    ...
    its tables silently won't appear in ``agentbox.core.data.metadata``
    — Alembic would miss them. This test catches that.
    """
    from agentbox.core.data._metadata import metadata as private
...
def test_shared_metadata_seen_by_legacy_schema_module() -> None:
    ...
    """
    from agentbox.core.data import schema
