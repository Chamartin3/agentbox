...
class TestAgentVersionsMixin:
    def test_create_version(self, svc) -> None:
    ...
    def test_latest_version_returns_newest(self, svc) -> None:
    ...
    def test_latest_version_returns_none_for_missing(self, svc) -> None:
        assert svc.latest_version("missing") is None

    def test_get_version(self, svc) -> None:
        _build_version(svc)
        v = svc.get_version("test-agent", 1)
        assert v is not None
        assert v["version"] == 1

    def test_get_version_returns_none_for_missing(self, svc) -> None:
    ...
    def test_list_agents_with_latest_empty(self, svc) -> None:
        assert svc.list_agents_with_latest() == []

    def test_list_versions_ordered_desc(self, svc) -> None:
    ...
    def test_list_versions_empty(self, svc) -> None:
        assert svc.list_versions("missing") == []

    def test_diff_versions(self, svc) -> None:
        svc.create_version(
            agent_id="diff-agent",
            source_path="/tmp/test.md",
            source_format="markdown",
    ...
    def test_diff_versions_raises_on_missing(self, svc) -> None:
        _build_version(svc)
        with pytest.raises(ValueError, match="version not found"):
            svc.diff_versions("test-agent", 1, 99)

    def test_add_and_list_comments(self, svc) -> None:
        v = _build_version(svc)
    ...
    def test_set_and_get_rating(self, svc) -> None:
        v = _build_version(svc)
...
class TestAgentLifecycle:
    def test_create_agent_writes_draft_v1_and_meta(self, svc) -> None:
    ...
    def test_create_agent_rejects_duplicate_id(self, svc) -> None:
        svc.create_agent(
            agent_id="duplicate",
    ...
    def test_create_agent_updates_orphaned_meta(self, svc, db) -> None:  # noqa: ANN001
    ...
    def test_publish_flips_draft_and_sets_active(self, svc) -> None:
        svc.create_agent(
            agent_id="pub-agent",
    ...
    def test_publish_appends_to_existing_changelog(self, svc) -> None:
        svc.create_agent(
