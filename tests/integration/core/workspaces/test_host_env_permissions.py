...
import pytest
from agentbox.core.mcp.inject import inject_host_env_mcp
...
class TestAuditLog:
    def test_audit_recorded_on_success(self, tmp_path: Path):
        ...
        with patch(
            "agentbox.core.mcp.servers.host_env.context.record_host_env_call"
        ) as rec:
            fs_read(str(target))
...
class TestExecutorInjection:
    def test_injects_host_env_into_mcp_config(self, tmp_path: Path):
        ...
        assert entry["args"] == ["-m", "agentbox.core.mcp.servers.host_env"]
        env = entry["env"]
