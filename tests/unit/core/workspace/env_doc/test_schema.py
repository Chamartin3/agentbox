...
class TestEnvDocContent:
    ...
    def test_project_name_is_required(self) -> None:
        with pytest.raises(ValidationError):
            EnvDocContent(overview="No name")
    ...
    def test_full_construction(self) -> None:
        content = EnvDocContent(
            ...
            sections=[Section(title="Extra", body_markdown="content")],
    ...
    def test_sections_default_to_visible(self) -> None:
        content = EnvDocContent(
            project_name="P",
            overview="O",
            sections=[Section(title="Default", body_markdown="x")],
        )
...
class TestReferenceLink:
    def test_minimal(self) -> None:
        link = ReferenceLink(label="API", url="https://api.example.com")
