"""Tests for WorkenvConfig value type."""
...
class TestWorkenvConfig:
    def test_round_trip_minimal(self) -> None:
        c = WorkenvConfig(name="test-workspace", description="A test")
        yaml_text = c.to_yaml()
        c2 = WorkenvConfig.from_yaml(yaml_text)
        assert c == c2
    ...
    def test_round_trip_empty_permissions(self) -> None:
        c = WorkenvConfig(name="empty-perms")
        yaml_text = c.to_yaml()
        c2 = WorkenvConfig.from_yaml(yaml_text)
        assert c == c2
