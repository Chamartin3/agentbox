"""Tests for the Token backend adapter."""
...
def test_render_stores_agent_meta() -> None:
    agent = _make_agent(
        runner=RunnerSpec(
            ...
            deps_factory="my_module:build_deps",
    ...
    assert rendered.agent_meta.get("deps_factory") == "my_module:build_deps"
    assert rendered.agent_meta.get("model") == "claude-sonnet-4-6"
...
def test_digest_changes_when_deps_factory_changes() -> None:
    adapter = TokenBackend()

    agent_a = _make_agent(
        runner=RunnerSpec(
            kind=RunnerKind.TOKEN,
            deps_factory="mod_a:build_a",
        )
    )
    agent_b = _make_agent(
        runner=RunnerSpec(
            kind=RunnerKind.TOKEN,
            deps_factory="mod_b:build_b",
        )
    )
