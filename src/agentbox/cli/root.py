...
# ---------------------------------------------------------------------------
# Backwards-compat aliases  (registered by __init__.py after importing app)
# ---------------------------------------------------------------------------


def register_backward_compat() -> None:
    """Add hidden aliases for renamed commands."""
    from agentbox.cli.agent import agent_ls

    @app.command("agents", hidden=True)
    def _agents_compat() -> None:
        console.print("[yellow]deprecated: use [bold]agent ls[/bold] instead[/yellow]")
        agent_ls()
    ...
    def _run_compat(
        ...
        console.print("[yellow]deprecated: use [bold]exec[/bold] instead[/yellow]")
        exec_cmd(agent, input_, api, session_id, follow)

    _legacy_ws = typer.Typer(help="[deprecated: use ws]", hidden=True)

    @_legacy_ws.command("list")
    def _ws_list_compat() -> None:
        console.print("[yellow]deprecated: use [bold]ws ls[/bold] instead[/yellow]")

    @_legacy_ws.command("path")
    def _ws_path_compat(agent: str) -> None:
        console.print(
            f"[yellow]deprecated: use [bold]ws path {agent}[/bold] instead[/yellow]"
        )

    @_legacy_ws.command("new")
    def _ws_new_compat(agent: str) -> None:
        console.print(
            f"[yellow]deprecated: use [bold]ws new {agent}[/bold] instead[/yellow]"
        )

    @_legacy_ws.command("reset")
    def _ws_reset_compat(agent: str) -> None:
        console.print(
            f"[yellow]deprecated: use [bold]ws reset {agent}[/bold] instead[/yellow]"
        )

    @_legacy_ws.command("edit")
    def _ws_edit_compat(agent: str) -> None:
        console.print(
            f"[yellow]deprecated: use [bold]ws edit {agent}[/bold] instead[/yellow]"
        )

    app.add_typer(_legacy_ws, name="workspace", hidden=True)
