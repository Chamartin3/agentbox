...
class AgentSyncMixin:
    """File-system sync metadata for agents.

    .. deprecated::
        Use ``Database.agent_sync`` manager methods instead.
    """
    ...
    def get_agent_sync(self, agent_id: str) -> dict | None:
        warnings.warn(
            "AgentSyncMixin.get_agent_sync is deprecated; use db.agent_sync.get(agent_id)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def upsert_agent_sync(
        ...
        warnings.warn(
            "AgentSyncMixin.upsert_agent_sync is deprecated; use db.agent_sync.create(...) or update(...)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def delete_agent_sync(self, agent_id: str) -> None:
        warnings.warn(
            "AgentSyncMixin.delete_agent_sync is deprecated; use db.agent_sync.delete(...)",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.begin() as conn:
