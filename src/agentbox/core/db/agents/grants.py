"""Mixin for agent-scoped tool grant CRUD."""
...
class AgentToolGrantsMixin:
    ...
    .. deprecated::
        Use ``Database.agent_tool_grants`` manager methods instead.
    """
    ...
    def grant_agent_tool(
        ...
        warnings.warn(
            "AgentToolGrantsMixin.grant_agent_tool is deprecated; use db.agent_tool_grants.create(...)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def revoke_agent_tool(
        ...
        warnings.warn(
            "AgentToolGrantsMixin.revoke_agent_tool is deprecated; use db.agent_tool_grants.update(...)",
            DeprecationWarning,
            stacklevel=2,
        )
        if len(changelog.strip()) < 3:
    ...
    def list_agent_tool_grants(
        ...
        warnings.warn(
            "AgentToolGrantsMixin.list_agent_tool_grants is deprecated; use db.agent_tool_grants.find(agent_id=...)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def list_active_grants(self, agent_id: str) -> set[str]:
        ...
        warnings.warn(
            "AgentToolGrantsMixin.list_active_grants is deprecated; use db.agent_tool_grants.find(agent_id=...)",
            DeprecationWarning,
            stacklevel=2,
        )
