"""Env doc mixin: one structured env doc per workspace, versioned.

Same audit pattern as prompts: every save bumps the version_number and
carries a non-empty changelog. `publish` flips the workspace-scoped
active pointer.
"""
...
class EnvDocsMixin:
    """
    .. deprecated::
        Use ``db.workspace_env_docs`` manager methods instead.
    ...
    def get_active_env_doc(self, workspace_id: str) -> EnvDocRow | None:
        warnings.warn(
            "EnvDocsMixin.get_active_env_doc is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def list_env_doc_versions(self, workspace_id: str) -> list[dict]:
        warnings.warn(
            "EnvDocsMixin.list_env_doc_versions is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.connect() as conn:
    ...
    def get_env_doc_version(self, version_id: str) -> dict | None:
        warnings.warn(
            "EnvDocsMixin.get_env_doc_version is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.connect() as conn:
    ...
    def save_env_doc(
        ...
        *,
        ...
        warnings.warn(
            "EnvDocsMixin.save_env_doc is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def publish_env_doc(self, workspace_id: str, version_id: str) -> dict:
        warnings.warn(
            "EnvDocsMixin.publish_env_doc is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.begin() as conn:
    ...
    def rollback_env_doc(
        ...
        *,
        ...
        warnings.warn(
            "EnvDocsMixin.rollback_env_doc is deprecated; use db.workspace_env_docs manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
