"""Host-env profile + workspace grant CRUD."""
...
class HostEnvMixin:
    ...
    def list_host_env_profiles(self) -> list[dict]:
        warnings.warn(
            "HostEnvMixin.list_host_env_profiles is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def get_host_env_profile(self, profile_id: str) -> dict | None:
        warnings.warn(
            "HostEnvMixin.get_host_env_profile is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.connect() as conn:
    ...
    def upsert_host_env_profile(
        ...
        *,
        ...
        warnings.warn(
            "HostEnvMixin.upsert_host_env_profile is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        now = now_iso()
    ...
    def delete_host_env_profile(self, profile_id: str) -> None:
        warnings.warn(
            "HostEnvMixin.delete_host_env_profile is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.begin() as conn:
    ...
    def get_workspace_host_env(self, workspace_id: str) -> dict | None:
        warnings.warn(
            "HostEnvMixin.get_workspace_host_env is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        with self.engine.connect() as conn:
    ...
    def set_workspace_host_env(
        ...
        *,
        ...
        warnings.warn(
            "HostEnvMixin.set_workspace_host_env is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def resolve_workspace_host_env(self, workspace_id: str) -> dict:
        warnings.warn(
            "HostEnvMixin.resolve_workspace_host_env is deprecated; use db.host_env_profiles manager instead.",
            DeprecationWarning,
            stacklevel=2,
        )
