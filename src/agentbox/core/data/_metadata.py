...
Every table in ``agentbox.core.data`` binds to this single instance so
...
domain packages (see ``docs/plans/24-core-data-consolidation.md``) without
fragmenting ``MetaData``.

Import ``metadata`` here from any module that defines a ``Table``.
External code should import ``metadata`` from ``agentbox.core.data`` —
