...
def import_repo_resources(store: SessionStore, root: Path) -> dict:
        ...
        for agent_dir in sorted(agents_root.iterdir()):
            ...
            for fname, tag in (("output_schema.json", "output_schema"), ("input_schema.json", "input_schema")):
                ...
                try:
                    ...
                    _safe(slug, lambda s=slug, n=agent_dir.name, t=tag, imp=schema_importer, p=fpath: _import_one(store, slug=s, type_="schema", display_name=f"{n} {t}", description=f"Imported from {p.relative_to(root)}", importer=imp, tags=(t, n), metadata_extra={"role": t, "agent_id": n}))
                except (OSError, ValueError):
                    _safe(slug, lambda p=fpath, s=slug, n=agent_dir.name, t=tag: _import_one(store, slug=s, type_="document", display_name=f"{n} {t}", description=f"Imported from {p.relative_to(root)}", importer=HostPathImporter(root=p), tags=(t, n), metadata_extra={"role": t, "agent_id": n, "schema_parse_failed": True}))
            sys_prompt = agent_dir / "prompts" / "system.md"
            if sys_prompt.is_file():
                slug = f"agent/{agent_dir.name}/system_prompt"
                _safe(slug, lambda p=sys_prompt, s=slug, n=agent_dir.name: _import_one(store, slug=s, type_=ResourceType.DOCUMENT, display_name=f"{n} system prompt", description=f"Imported from {p.relative_to(root)}", importer=HostPathImporter(root=p), tags=("system_prompt", n), metadata_extra={"role": "system_fragment", "agent_id": n}))
    ...
    if shared_root.is_dir():
        for scope_dir in sorted(shared_root.iterdir()):
            ...
            _safe(slug, lambda d=scope_dir, s=slug: _import_one(store, slug=s, type_=ResourceType.FOLDER, display_name=d.name, description=f"Imported from {d.relative_to(root)}", importer=HostPathImporter(root=d), tags=("reference", d.name)))
...
def sweep_workspace_skill_bindings(store: SessionStore, manifest) -> dict:
    ...
    for ws in manifest.workspaces:
        ...
        if not bindings:
            continue
        try:
            store.replace_workspace_file_bindings(ws.name, bindings, reason=DEFAULT_BINDING_REASON, actor="boot_sweep")
            workspaces_wired += 1
    ...
    for agent in manifest.agents:
        ...
        for idx, ref in enumerate(comp.references):
            ...
            def _do_doc(p=fpath, s=slug, dn=display_name):
                return _import_one(store, slug=s, type_=ResourceType.DOCUMENT, display_name=dn, description=f"Imported from {p.relative_to(root)}" if root in p.parents else f"Imported from {p}", importer=HostPathImporter(root=p), tags=("reference",))
            try:
                action, _ = _do_doc()
                if action == "created":
                    resources_created += 1
        ...
        if not bindings:
            continue
        try:
            store.replace_prompt_bindings(agent.id, bindings, reason="boot: auto-imported from composition.references", actor="boot_sweep")
            agents_wired += 1
            bindings_added += len(bindings)
        except Exception:
